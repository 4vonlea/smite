            ...
            <div v-if="!loading">
				...
				<div class="row  table-responsive">
					...
					<table class="table table-bordered text-light">
						<thead>
							<tr style="background-color: #0fa091;" class="text-center">
						...
						<tbody v-else>
							<tr v-for="item in transaction">
								...
								<td>
									<v-button class="btn btn-primary" @click="detailTransaction(item,$event)" icon="fa fa-list">Click for detail</v-button>
				...
				<div class="row table-responsive mt-3">
					<h4>Current Cart</h4>
					<div v-if="!cart" class="alert alert-warning text-center mt-2 text-capitalize">
						You haven't selected an event to add
					</div>
					<table v-else class="table text-light">
						<thead>
							<th class="text-light"></th>
							<th class="text-light">Event Name</th>
							<th class="text-light">Pricing</th>
						</thead>
						<tbody>
					...
					<div class="modal-content"  style="background-color: #212428;">
						...
						<div class="modal-body table-responsive">
							<table class="table table-bordered text-light">
								<tbody>
									<tr>
										<th class="text-light">Invoice ID</th>
										<td>{{ detailModel.id }}</td>
										<th class="text-light">Invoice Date</th>
									...
									<tr>
										<th class="text-light">Bill To</th>
									...
									<tr>
										<th class="text-center text-light" colspan="4">Detail</th>
									</tr>
									<tr>
										<th class="text-light" colspan="2">Event Name</th>
										<th class="text-light" colspan="2">Price</th>
									</tr>
							...
							<div v-if="detailModel.status_payment == 'pending' && detailModel.channel == 'ESPAY'">
								<table class="table table-bordered text-light" v-if="detailEspay.product_value">
									<tr>
										<th class="text-light">Bank/Vendor Name</th>
										<td>
										...
										<div class="card card-achievement p-2">
												...
												<p class="card-text table-responsive">
													<table>
														...
														<tr>
															<th class="text-light">Account holder's name</th>
															<td>:</td>
								...
								<div class="col-sm-6" v-for="account in manual_payment.banks">
									<div class="card card-achievement">
										<div class="card-body">
											...
											<p class="card-text">
												<table>
													<tr><th class="text-light">Account Number</th><td>:</td><td>{{ account.no_rekening }}</td></tr>												
													<tr><th class="text-light">Account Holder</th><td>:</td><td>{{ account.holder }}</td></tr>												
												</table>
											</p>
										</div>
									</div>
			...
			$.get(this.appUrl + "/member/payment/check_payment").always(function () {
				$.post(page.baseUrl + "get_transaction", null, function (res) {
					if (res.status) {
						...
						if (res.paymentMethod.length == 1) {
							page.selectedPaymentMethod = 1; //page.paymentMethod[1];
						}
