				...
				<div class="card shadow border-0">
					<div class="card-header bg-white pb-5">
						<div class="row">
							<div class="col-6">
								<h3>
									Presence Check Event : {{ pageCheck.event.name }} |
									<select v-if="pageCheck.event.session.length > 0" v-model="pageCheck.selectedSession">
										<option value="">-- Select Session --</option>
										<option v-for="ses in pageCheck.event.session" :value="ses">{{ ses }}</option>
									</select>
								</h3>
							</div>
					...
					<div v-if="pageCheck.selectedSession || pageCheck.event.session.length == 0">
						<div class="table-responsive">
							<table class="table">
								<tr>
						...
						<div class="col-sm-12 text-center">
							...
							<video v-show="checkMode == 1" style="border:solid 3px;max-width: 500px;width: 100%" muted playsinline id="qr-video" ref="qrVideo"></video>
						</div>
					</div>
					<div v-else class="col-sm-12">
						<div class="alert alert-info text-center">
							Please select event session first
						</div>
					</div>
				</div>
			</div>
					...
					<div class="card-body">
						...
						<div class="table-responsive">
							...
							<div style="position: relative">
								...
								<vuetable ref="vuetable" :api-mode="false" :fields="detail.column" :data="detail" :data-total="dataCount" :data-manager="dataManager" data-path="data" pagination-path="pagination" :per-page="pageSize" :css="css.table" @vuetable:pagination-data="onPaginationData">
								</vuetable>
	...
	var app = new Vue({
		...
		methods: {
			...
			fetchDetail() {
				...
				$.post("<?= base_url('admin/presence/get_detail'); ?>", {
					id: this.detail.event.id_event
				}, function(res) {
				...
				$.each(app.pageCheck.data, function(i, r) {
					if (r.id == result) {
						...
						Swal.fire({
							...
							type: 'success',
							html: `<p>Presence of <b>${r.fullname}</b> As <b>${r.status_member}</b></p>` +
						...
						$.post("<?= base_url('admin/presence/save'); ?>", {
							...
							session: app.pageCheck.selectedSession,
			...
			presencePage(row, ev) {
				...
				$.post("<?= base_url('admin/presence/get_data'); ?>", {
					id: row.id_event
				}, function(res) {
					if (res.status) {
						let session = [];
