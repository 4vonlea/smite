...
class Presence extends Admin_Controller
{
	...
	public function save()
	{
		...
		$this->Presence_m->addPresenceNow($this->input->post('member_id'), $this->input->post('event_id'), $this->input->post('session'));
	}
	...
	public function get_event_transaction()
	{
		...
		$this->load->model(["Transaction_m", "Presence_m"]);
		$data = $this->Transaction_m->getFollowedEvent($id);
		$status = count($data) > 0;
		$presenceData = [];
		if ($status) {
			$presenceData = $this->Presence_m->getPresenceMemberToday($data[0]['member_id']);
		}
		$message = $status ? "Success in retrieving data" : "The transaction is not found or has not been settled";
		$this->output
			->set_content_type("application/json")
			->_display(json_encode(['status' => $status, 'data' => $data, 'presenceData' => $presenceData, 'message' => $message]));
	}
